...
def generate_example(found_func_names, func_node):
    ...
    # Randomly choose two non-builtin functions
    ...
    # good function with swapped non-builtins
...
def process_function(func_node):
    # find calls to two non-builtin functions
    global num_generated_funcs

    found_func_names = set()
    possible_internal_func_names = set()
    def traverse(node):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id not in target_func_names:
            found_func_names.add(node.func.id)
        if isinstance(node, ast.FunctionDef):
            possible_internal_func_names.add(node.name)
        if hasattr(node, "target"):
            if isinstance(node.target, ast.Name):
                possible_internal_func_names.add(node.target.id)
        if hasattr(node, "targets"):
            for target in node.targets:
                if isinstance(target, ast.Name):
                    possible_internal_func_names.add(target.id)
        for child in ast.iter_child_nodes(node):
            traverse(child)
    traverse(func_node)

    # remove names of functions that are possibly defined inside the current top-level function
    found_func_names = found_func_names - possible_internal_func_names
    # num of non-builtins check
